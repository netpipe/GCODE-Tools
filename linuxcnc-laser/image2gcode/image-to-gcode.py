...
class Converter:
    ...
    def convert(self):
        ...
        if self.roughing_delta and self.roughing_offset:
            ...
            self.image = numarray.zeros((w,h), "Float32")
            for j in range(0, w):
                progress(j,w)
                for i in range(0, h):
...
def ui(im, nim, im_name):
  ...
  #  rs274.options.install(app)
    app.tk.call("source", os.path.join("combobox.tcl"))
...
def main():
    ...
    w, h = im.size
    #print type(im.tostring().decode('iso-8859-1'))
    nim = numarray.fromstring(im.tostring(), 'UInt8').astype('Float32')
    nim.shape = (w, h)
